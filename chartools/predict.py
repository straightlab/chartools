import numpy as np
from .charcounter import Chartable
from scipy import sparse

class Predictor:

    def __init__(self, bkg, proj, bychr=True, leakage_matrix=None, proj_rna=None, cismodel=None, gene_strand_dict=None):
        
        self.proj=proj
        self.proj_rna=proj_rna
        self.cismodel=cismodel
        self.gene_strand_dict=gene_strand_dict
        self.leakage_matrix=leakage_matrix

        #B_bychr=bkg.normalize_row_bychr()
        self.B_bychr=((bkg.normalize_row_bychr().counts.tocsc())*self.proj['counting_matrix']).tocsr()

        
        self.B_bytrans=((bkg.normalize_row_bytrans().counts.tocsc())*self.proj['counting_matrix']).tocsr()

    @staticmethod
    def get_leakage_matrix(d): # d is a full chartable_binned used for generating leakage_matrix
        return 0

    def _new_chartable(self, d, counts):
        out=Chartable()
        out.nannot=d.nannot
        out.annot_chr=d.annot_chr.copy()
        out.annot_dict = {k:v for k, v in d.annot_dict.items()}
        out.annot_LUT=[k for k in d.annot_LUT]

        out.chr_dict=d.chr_dict.copy()
        out.chr_LUT=[k for k in d.chr_LUT]
        out.chr_len_vec=d.chr_len_vec.copy()
        out.nchr=d.nchr

        out.chr_blocks=self.proj['chr_blocks']

        out.nbins=counts.shape[1]
        out.bins_LUT=self.proj['feature_name']
        out.counts=counts

        return out

    @staticmethod
    def predict_N_by_chr(d, leakage_matrix=None, include_cis=False):
        if leakage_matrix is None: # the S matrix takes up the observed values
            d_trans=d
            if not include_cis:
                _, d_trans, _ = d.cis_trans()
            
            S=d_trans.N_bychr()
        else: 
            d_cis, _, _ = d.cis_trans()
            L=d_cis.get_chr_matrix(withcounts=True) #locus matrix ngenes * nchromosomes
            #reference is always cis counts which is why we use d_cis
                
            K=leakage_matrix #should have 1 at diagonal
            if not include_cis:
                K=leakage_matrix.copy()
    ...
    @staticmethod
    def predict_N_by_trans(d, leakage_matrix=None, include_cis=False):
        # the S matrix directly : ngenes x 2 (cis, trans)
        dcis, _, S=d.cis_trans(format='matrix')

        if not leakage_matrix is None: # the S matrix takes up the observed values
            L=dcis.get_chr_matrix(withcounts=True) #ngenes*nchr

            S[:,1]=L*leakage_matrix #(leakage matrix is nchr x 1)

        return S

    def predict_trans(self, d, by_chr=True, input_is_N=False, include_cis=False, use_leakage=False, return_chartable=False):
            
            leakage_matrix=None if (not use_leakage) else self.leakage_matrix
            
            if by_chr:
                if input_is_N: 
                    S=d #for now this doesn't respet the include_cis
                else:
                    S=Predictor.predict_N_by_chr(d, leakage_matrix=leakage_matrix, include_cis=include_cis)
                
                out_counts=S*(self.B_bychr)

            else: #by trans
                if input_is_N: 
                    S=d #for now this doesn't respet the include_cis
                else:
                    S=Predictor.predict_N_by_trans(d, leakage_matrix=leakage_matrix, include_cis=include_cis)
                
                L=d.get_chr_matrix(withcounts=False) #locus matrix ngenes * nchromosomes
                L_trans=sparse.csr_matrix((S[:,1].ravel(), L.indices, L.indptr), shape=L.shape)
                out_counts=L_trans*self.B_bytrans

                if include_cis:
                    L_cis=sparse.csr_matrix((S[:,0].ravel(), L.indices, L.indptr), shape=L.shape)
                    out_counts += L_cis*self.B_bychr

            if return_chartable:
                out=self._new_chartable(d, out_counts)
                return out, S
            else:
                return out_counts, S

    # def predict_cis(self, r, cismodel=None, gene_strand_dict=None, adjust_local_accessibility=True, return_chartable):
        
    #     m=cismodel if (not cismodel is None) else self.cismodel

    #     if not m is None:

    #         pred=r.predict_cis_withinGI(cismodel, self.projR, gene_strand_dict=gene_strand_dict, return_chartable=False)

    #         if adjust_local_accessibility:
            ...
            # def get_trans_matrix_norm(O, rows_chr_ids, col_chr_blocks=None):
            #     # returns Nj / N in block j
            #     # if col_chr_blocks is None:
            #     #     col_chr_blocks=np.arange(())
            #     # rows_chr_ids=genes_df['chr_id'].loc[cmap.index].values.astype(int)
            ...
            # def probe_gene_cape(pairs, chr_len_dict, chr_to_ID, gene_to_ID, tx_dict, bins=np.arange(0,1000000,1000), bygene=True, qmin_rna=0, qmin_dna=15, noambiv=True, nmax=0): #dict of vectors, which are targeted chr, first numer is # match,
            # # second is # of dna match'
            #     if noambiv:
            #         cmd="cat "+pairs+" | awk '($25==1 && $8>"+str(qmin_rna)+" && $9>"+str(qmin_dna)+"){print $18, $4, substr($2,3), $5, $3}'"
            #     else:
            ...
            #     ngenes=max([v[1] for _, v in gene_to_ID.items()])+1
            ...
            #             # tgt_binid=int(np.floor((read_data[3]-1)/binsize))
            #             # tgt_pos=int(read_data[3])-1
            #             #TODO: check position base for gff and in pairs file
            #             tss=int(T_annot[2]) 
            #             src_pos=int(read_data[4])-1
                        
            #             out[gene_ID, np.searchsorted(bins,np.abs(src_pos-tss),'right')-1]+=1
            ...
            # def get_gene_pos(gene_to_id2, tx_dict, chr_dict2):
            #     nchr=max(v for v in chr_dict2.values())
            #     ngenes_bychr=np.zeros(nchr, int)
            #     for _, v in gene_to_id2.items():
            #         ngenes_bychr[v[0]]=max(ngenes_bychr[v[0]],v[1]+1)
                
            #     gene_pos=[np.zeros(negens_bychr[i]) for i in range(nchr)]
            #     for k, v in gene_to_id2.items():
            #         genedata=tx_dict[k]
            #         if genedata[1]=="-":
            #             gene_pos[v[0]][v[1]]=-genedata[2]
            #         else:
            #             gene_pos[v[0]][v[1]]=genedata[2]

            #     #genepos=[[[g1_chr1, g2_chr1],....]]
            # def get_expected_INinterval_fromCIS(chrID, start, stop, gene_cap, gene_cap_dist, gene_pos, R): #R is the cumul distribution of flight distance
            #     n=len(gene_pos[chrID])
            #     p=len(gene_cap[chrID])
            #     E=np.zeros(n)
            #     theta0=start-gene_pos[chrID]
            #     theta1=theta0+(stop-start)
            #     for i in range(p-1):
            #         E+=R(theta1-gene_cap_dist[i])
            #         E+=R(theta1-gene_cap_dist[i+1])
            #         E-=R(theta0-gene_cap_dist[i])
            #         E-=R(theta0-gene_cap[i+1])
            #     E=E/2

            # def expected_fast(pairs, chr_len_dict, chr_to_ID, gene_to_ID, tx_dict, bins, gene_cap, bygene=True, qmin_rna=0, qmin_dna=15, noambiv=True, nmax=0):
            # if noambiv:
            #         cmd="cat "+pairs+" | awk '($25==1 && $8>"+str(qmin_rna)+" && $9>"+str(qmin_dna)+"){print $18, $4, substr($2,3), $5, $3}'"
            #     else:
            #         cmd = "cat "+pairs+" | awk '($8>"+str(qmin_rna)+" && $9>"+str(qmin_dna)+"){print $18, $4, substr($2,3), $5, $3}'"
