...
def bam2pairs_record(rR,rD,nr,nd,invertFlag,triangular_mode):
    ...
    annot_refD=(rD.get_tag('ar') if rD.has_tag('ar') else "*")
    annot_posD=str(rD.get_tag('ap') if rD.has_tag('ap') else 0)
    annot_nameD=(rD.get_tag('an') if rD.has_tag('an') else "*")
    annot_typeD=(rD.get_tag('at') if rD.has_tag('at') else "*")
    # annot_strand=(rR.get_tag('as') if rR.has_tag('as') else "*")
    # annot_length=(str(rR.get_tag('al')) if rR.has_tag('al') else "0")

    
    annot_gSD=str(rD.get_tag('gS') if rD.has_tag('aS') else 0)
    annot_aSD=str(rD.get_tag('aS') if rD.has_tag('aS') else 0)
    annot_aiD=str(rD.get_tag('ai') if rD.has_tag('ai') else 0)
    annot_aID=str(rD.get_tag('aI') if rD.has_tag('aI') else 0)
    annot_aGD=str(rD.get_tag('aG') if rD.has_tag('aG') else "*")
    ...
    s=""
    if (((triangular_mode==2) or (triangular_mode==0)) and is_lowtrig): #reverse the R and D, do not add prefix
        s="\t".join([
            rD.query_name,rD.reference_name,str(posD),rR.reference_name,str(posR),sD,sR, 
            str(rD.mapping_quality),str(rR.mapping_quality), str(rD.flag), str(rR.flag), gap2bridgeD,gap2bridgeR, str(rD.query_alignment_length), 
            str(rR.query_alignment_length), nD, nR, annot_refD, annot_posD, annot_nameD, annot_typeD, annot_gSD, annot_aSD, annot_aiD, annot_aID, annot_aGD, annot_ref, annot_pos, annot_name, annot_type, annot_gS, annot_aS, annot_ai, annot_aI, annot_aG])
    else:
        ...
        # 15=nR, 16=nD, 17=annot_ref(ENST), 18=annot_pos, 19=annot_name(ex: gapdh), 20=annot_type, 21=gS (#of genomic aligmnet with compatible annotation RNA side), 22=aS (#of annotations), 23=ai, 24=aI, 25=ENSG. 26=annot_ref(ENST), 27=annot_pos, 28=annot_name(ex: gapdh), 29=annot_type, 30=gS (#of genomic aligmnet with compatible annotation RNA side), 31=aS (#of annotations), 32=ai, 33=aI, 34=ENSG
        s="\t".join([
            ...
            str(rD.query_alignment_length), nR, nD, annot_ref, annot_pos, annot_name, annot_type, annot_gS, annot_aS, annot_ai, annot_aI, annot_aG, annot_refD, annot_posD, annot_nameD, annot_typeD, annot_gSD, annot_aSD, annot_aiD, annot_aID, annot_aGD])
